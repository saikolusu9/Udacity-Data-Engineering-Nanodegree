...
| :---------------: | :----------------------------------------------------------: |
|       dags        | Folder where DAGs and SubDAGS are stored |
|      images       |  Folder where images are stored  |
|  plugins/helpers  |  This folder contains a SQL helper class for easy querying         |
| plugins/operators | This folder contains the custom operator to perform the DAG tasks     |
| create_tables.sql | This folder contains SQL commands to create the necessary tables on Redshift |
|      README       |                         Readme file                          |
...
2. Set up `AWS Connection` and `Redshift Connection` to Airflow using necessary values
3. In Airflow, turn the DAG execution ON
4. View the Web UI for detailed insights about the operation